...
**问题描述：**
- 标的物字段中出现大量"首页政采法规购买服务监督检查信息公告国际专栏"等页面导航文字
- 这些无效内容被误提取为标的物

**修复方案：**
- 增强 `_is_invalid_subject()` 方法的过滤逻辑
- 添加页面导航/页脚文字过滤（首页、政采、法规、购买服务等）
- 添加招投标流程相关词汇过滤（询价公告、竞争性谈判等）
- 添加操作按钮/链接词汇过滤（查看更多、点击、返回等）
- 添加导航关键词组合检测（4个以上导航词视为无效）

**修复效果：**
- 修复前：大量记录包含无效导航文字
- 修复后：50条记录中无效内容记录数为0，有效记录比例100%
...
| 文件 | 修改内容 |
|------|----------|
| `src/fetchers/ccgp_fetcher.py` | 增强标的物过滤，修复分页解析，添加重试机制，支持获取所有结果 |
| `src/config.py` | 允许 max_results 为0 |
| `src/main.py` | 添加 `--all` 参数，优化输出信息 |
| `README.md` | 更新功能特性、使用示例、参数说明、更新日志 |
| `SKILL.md` | 更新功能特性、使用示例、参数说明、注意事项 |
...
**提交记录：**
- `c6f0c4e` - fix: 增强标的物无效内容过滤
- `9d30da6` - docs: 更新使用文档和项目总结
