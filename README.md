...
| 参数 | 简写 | 说明 | 默认值 |
|------|------|------|--------|
| --keywords | -k | 搜索关键词列表 | 眼科 皮肤科 |
| --days | -d | 搜索最近多少天 | 7 |
| --output | -o | 输出文件路径 | medical_tenders_时间戳.xlsx |
| --max-results | -m | 每个关键词最大结果数 | 100 |
| --max-pages | -p | 每个关键词最大抓取页数 | 5 |
| --timeout | -t | HTTP请求超时时间(秒) | 30 |
| --multi-sheet | | 按关键词分多个工作表 | False |
| --no-summary | | 不包含汇总统计 | False |
...
1. **请求频率**：工具内置了1-2秒的请求间隔，避免对目标网站造成压力
